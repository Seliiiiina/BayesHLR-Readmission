...
=========================
Specialty regrouping and feature engineering (design matrix construction) for
Bayesian Hierarchical Logistic Regression on 30-Day Hospital Readmission.

Covers Pipeline Tasks 3-4:
  Task 3: Specialty regrouping
  Task 4: Variable construction (design matrix)

Input:
  - data_processed/cleaned_intermediate.csv   (from 01_clean_data.py)
...
def log(msg):
    """Append a message to the log and print to stdout."""
...
intermediate_path = DATA_PROCESSED / "cleaned_intermediate.csv"
if not intermediate_path.exists():
    sys.exit(f"ERROR: Cannot find cleaned data at {intermediate_path}\n"
             f"       Run 01_clean_data.py first.")

df = pd.read_csv(intermediate_path)
log(f"Loaded cleaned intermediate data: {df.shape[0]} rows x {df.shape[1]} cols")
...
log_path = DATA_PROCESSED / "feature_engineering_log.txt"
...
print(f"02_feature_engineering.py completed successfully.")
print(f"{'='*70}")
