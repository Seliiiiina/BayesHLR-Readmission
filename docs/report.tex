...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Continuous predictors are standardised to mean 0 and variance 1 to
    stabilise computation and make coefficient scales comparable.
  \item Race and primary-diagnosis group are encoded by reference-category
    indicators (reference levels: Caucasian and Circulatory, respectively),
    while \texttt{A1C\_level} and \texttt{glu\_level} are treated as ordinal
    covariates.
...
The intercept appears in every observation through
$\eta_i = \alpha + \bx_i^\top\bbeta + u_{j[i]}$, so its log-target contains
the full likelihood plus the Gaussian prior penalty:
\begin{equation}\label{eq:fc-alpha}
  h(\alpha) = \sum_{i=1}^{N}
    \bigl[y_i\,\eta_i - \log(1+e^{\eta_i})\bigr]
  - \frac{\alpha^2}{2\sigma_\alpha^2}.
\end{equation}
The first term aggregates information from all $N$ encounters; the second
anchors the baseline log-odds near zero.
...
Each fixed effect is updated one coordinate at a time.  Because $\eta_i$
depends on $\beta_k$ only through $x_{ik}\beta_k$, the block-specific
log-target is:
...
\end{equation}
All other coefficients $\beta_{k'}$ ($k' \neq k$) are held at their current
values during this coordinate-wise Metropolis update.
...
The random intercept $u_j$ affects only observations belonging to specialty
$j$.  Consequently, its log-target involves the likelihood contribution from
that specialty alone, plus the Gaussian prior penalty from $p(\bu \mid \tau)$:
\begin{equation}\label{eq:fc-u}
  h(u_j) = \sum_{i:\,j[i]=j}
    \bigl[y_i\,\eta_i - \log(1+e^{\eta_i})\bigr]
  - \frac{u_j^2}{2\tau^2}.
\end{equation}
This locality is computationally important: the update costs $O(n_j)$ rather
than $O(N)$.
...
Because $\tau > 0$, we work on the unconstrained scale
$\phi = \log\tau$.  Under the transformation $\tau = e^\phi$, the Jacobian is
...
\begin{remark}[Why $\tau$ does not appear in the likelihood term]
Conditional on $\bu$, the data $\by$ are independent of $\tau$:
the likelihood depends on the specialty effects $u_j$, but not on their
population scale directly.  As a result, the data inform $\tau$ only through
the posterior spread of the $u_j$'s.  Large dispersion in $\bu$ pushes $\tau$
upward, while strong shrinkage of $\bu$ toward zero pulls $\tau$ downward.
\end{remark}
...
which updates one block at a time using the log-targets derived in
...
One iteration $t \to t+1$ consists of the following five stages.
...
Computationally, we avoid recomputing $\eta$ from scratch:
$\eta_i^* = \eta_i^{(t)} + (\alpha^* - \alpha^{(t)})$ shifts every entry by a
constant, so the update costs $O(N)$.
...
\end{equation}
This move costs $O(J)$ with no likelihood evaluation, so we repeat it
$R = 10$ times per iteration to accelerate mixing along the weakly
identified ridge.
...
all other parameters.  This defines a valid Metropolis-within-Gibbs chain whose stationary
...
manageable scalar updates, each with its own tuned proposal, and yields
stable acceptance rates in practice.
...
\texttt{src/05b\_tune\_proposals.py}:

\begin{enumerate}
  \item \textbf{Pilot runs.}  Run short pilot chains of 2,000 iterations on a
    random subsample of $n = 20{,}000$ observations to reduce tuning cost.
  \item \textbf{Adaptation rule.}  After each pilot round, compute the
    empirical acceptance rate for each parameter block.  If the rate is above
    the target range, multiply $\sigma_{\text{prop}}$ by 1.5; if it is below
    range, multiply by 0.5.
  \item \textbf{Per-coordinate tuning.}  Separate proposal SDs are maintained
    ...
    race dummy).
  \item \textbf{Stopping rule.}  Tuning stops when either all parameters fall
    in the target acceptance range $[0.20, 0.50]$, or at least 90\% of them do,
...
from a log-normal draw centered near 1.  The production configuration is:
\begin{itemize}
  \item \textbf{Number of chains:} 4.
