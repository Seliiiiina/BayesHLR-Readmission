...
\newcommand{\sectionlead}[1]{%
  \vspace{0.2ex}%
  \noindent\begin{minipage}{\linewidth}\small\itshape #1\end{minipage}\par
  \vspace{0.6ex}%
}
...
\newcommand{\takeawaybox}[2]{%
  \begin{center}
    ...
    \fbox{\begin{minipage}{0.94\linewidth}\small\textbf{#1.} #2\end{minipage}}%
    \endgroup
  \end{center}
}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]{fig1_specialty_distribution.png}
\caption{Distribution of encounter counts across the 19 specialty groups in the
analysis cohort. The pronounced group-size imbalance motivates partial pooling
in the hierarchical model.}
\label{fig:specialty-distribution}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]{fig3_raw_readmission_by_specialty.png}
\caption{Raw 30-day readmission rates by specialty, with sample sizes shown for
context. The wide spread in unadjusted rates motivates modeling
specialty-level heterogeneity while accounting for unequal group sizes.}
\label{fig:raw-readmission-by-specialty}
\end{figure}
...
\sectionlead{This section defines the hierarchical logistic model, introduces
the notation used throughout the derivations, and highlights how the shared
scale $\tau$ induces partial pooling across specialties.}
...
\takeawaybox{Hierarchy at a Glance}{Level 1 models readmission outcomes
through a logistic regression, Level 2 places exchangeable Gaussian priors on
the specialty effects $u_j$, and Level 3 learns their dispersion through the
hyperparameter $\tau$.}
...
\begin{itemize}
  ...
  \item \textbf{Hyperprior on $\tau$}: $\HalfNormal(0,1)$ concentrates mass
    ...
    patient-level predictors.  On the log-odds scale, $\tau = 1$ implies that
    a specialty lying one standard deviation above the population mean has
    odds about $e^1 \approx 2.7$ times as large as the population-average
    specialty, which is already substantial.
\end{itemize}
...
\takeawaybox{Partial Pooling Intuition}{Large specialties are driven mainly by
their own data, whereas small specialties borrow more strength from the common
distribution $u_j \mid \tau \sim \Normal(0,\tau^2)$.  The amount of borrowing
is therefore learned rather than fixed in advance.}
...
\sectionlead{We move from the factorised joint density to the log-posterior,
then isolate the terms needed for each Metropolis-within-Gibbs update.  The
guiding rule is simple: retain every term involving the block being updated
and absorb everything else into the normalising constant.}

\takeawaybox{Derivation Flow}{The posterior construction proceeds in three
steps: write the joint density, drop parameter-free constants to obtain the
log-posterior, and collect only the relevant terms for the current block.}
...
\takeawaybox{Reading Each Block Target}{For any parameter block $\theta$, the
working log-target $h(\theta)$ is obtained by retaining all terms in
\eqref{eq:logpost} that depend on $\theta$ and treating the remainder as an
additive constant.}
...
\textit{Target.} Collecting terms (I) and (II) that depend on $\alpha$
(recall $\eta_i = \alpha + \bx_i^\top\bbeta + u_{j[i]}$) gives
\begin{equation}\label{eq:fc-alpha}
  h(\alpha) = \sum_{i=1}^{N}
    \bigl[y_i\,\eta_i - \log(1+e^{\eta_i})\bigr]
  - \frac{\alpha^2}{2\sigma_\alpha^2}.
\end{equation}
The first term is the full log-likelihood, while the second is the Gaussian
prior penalty.

\textit{Proposal.}
\[
  \alpha^* = \alpha^{(t)} + \epsilon,
  \qquad \epsilon \sim \Normal(0,\sigma_{\alpha,\text{prop}}^2).
\]

\textit{Acceptance ratio.}
\[
  \log R = h(\alpha^*) - h(\alpha^{(t)}).
\]

\paragraph{Block 2: $\beta_k$ (for each $k = 1,\dots,K$).}
\textit{Target.} Since $\eta_i$ depends linearly on $\beta_k$ through
$x_{ik}\beta_k$, the corresponding log-target is
\begin{equation}\label{eq:fc-beta}
  h(\beta_k) = \sum_{i=1}^{N}
    \bigl[y_i\,\eta_i - \log(1+e^{\eta_i})\bigr]
  - \frac{\beta_k^2}{2s_\beta^2}.
\end{equation}
Each $\beta_k$ is updated individually, holding all other coefficients
$\beta_{k'}$ ($k' \neq k$) fixed at their current values.

\textit{Proposal.}
\[
  \beta_k^* = \beta_k^{(t)} + \epsilon,
  \qquad \epsilon \sim \Normal(0,\sigma_{\beta_k,\text{prop}}^2).
\]
...
\textit{Target.} The key simplification is local dependence: $u_j$ enters
$\eta_i$ only for observations in group $j$.  Hence only group-$j$
observations contribute to the likelihood term:
\begin{equation}\label{eq:fc-u}
  h(u_j) = \sum_{i:\,j[i]=j}
    \bigl[y_i\,\eta_i - \log(1+e^{\eta_i})\bigr]
  - \frac{u_j^2}{2\tau^2}.
\end{equation}
This reduces the computational cost from $O(N)$ to $O(n_j)$ per update.

\textit{Proposal.}
\[
  u_j^* = u_j^{(t)} + \epsilon,
  \qquad \epsilon \sim \Normal(0,\sigma_{u_j,\text{prop}}^2).
\]
...
\textit{Change of variable.} Since $\tau > 0$, we work on the unconstrained
scale $\phi = \log\tau$.  The Jacobian of the transformation is
$\bigl|\frac{d\tau}{d\phi}\bigr| = e^\phi = \tau$, contributing an additive
$+\phi$ term to the log-density.

\textit{Target.} Collecting terms (IV) and (V) from~\eqref{eq:logpost},
substituting $\tau = e^\phi$, and adding the log-Jacobian yields
\begin{equation}
  h(\phi)
  = -J\,\phi
...
\[
  \phi^* = \phi^{(t)} + \epsilon,
  \qquad \epsilon \sim \Normal(0,\sigma_{\phi,\text{prop}}^2).
\]
...
hierarchical structure: $\tau$ learns from the pattern of group effects rather
than from individual observations directly.
\end{remark}

\paragraph{Sensitivity variant: Half-Cauchy hyperprior.}
\textit{Target.} Under $\tau \sim \HalfCauchy(0, s_\tau)$, the log-prior on
$\tau$ becomes $-\log(1 + \tau^2/s_\tau^2) + \text{const.}$ instead of
$-\tau^2/(2s_\tau^2)$.  On the $\phi$ scale, this replaces the final term
...
\sectionlead{This section turns the posterior targets into an implementable
sampler.  The presentation separates generic Metropolis--Hastings mechanics
from the model-specific block structure so that the computational logic is easy
to scan before reading implementation details.}
...
\takeawaybox{Sampler Design}{Blocks~1--4 update scalar coordinates or the
transformed scale parameter, while Block~5 targets the weakly identified
translation ridge between $\alpha$ and $\bu$ to improve mixing at negligible
additional cost.}

\begin{table}[H]
\centering
\small
\caption{Overview of the MwG update blocks.}
\label{tab:mwg-overview}
\begin{tabular}{p{0.08\linewidth} p{0.19\linewidth} p{0.33\linewidth} p{0.12\linewidth} p{0.16\linewidth}}
\toprule
Block & Parameter(s) & Proposal structure & Cost & Likelihood term? \\
\midrule
1 & $\alpha$ & Scalar random walk on the intercept & $O(N)$ & Yes \\
2 & Each $\beta_k$ & Scalar random walk on one fixed effect at a time & $O(N)$ per $k$ & Yes \\
3 & Each $u_j$ & Scalar random walk using only group-$j$ observations & $O(n_j)$ & Yes, local only \\
4 & $\phi = \log\tau$ & Scalar random walk on the unconstrained scale & $O(J)$ & No \\
5 & $(\alpha,\bu)$ jointly & Translation move $\alpha \mapsto \alpha+\delta$, $u_j \mapsto u_j-\delta$ & $O(J)$ ratio & No \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Motivation.} The intercept $\alpha$ and the random intercepts $\bu$ are weakly
...
\textit{Proposal.} Propose $\delta \sim \Normal(0, \sigma_{\text{blk}}^2)$,
set $\alpha^* = \alpha + \delta$ and $u_j^* = u_j - \delta$ for all $j$.  Since
...
\end{equation}
Because this ratio is likelihood-free, we repeat the move $R = 10$ times per
iteration at negligible additional cost to accelerate mixing along the weakly
identified ridge.
...
\begin{enumerate}
  \item \textbf{Pilot runs.}  Run up to 20 rounds of short pilot chains (2,000
  ...
  \item \textbf{Adaptation rule.}  After each pilot round, compute the
    empirical acceptance rate for each parameter.  We target the interval
    $[0.20, 0.50]$, centred around the scalar-optimal benchmark of roughly
    0.44 from \citep{roberts2001optimal}.  If the acceptance rate exceeds
    0.50, multiply $\sigma_{\text{prop}}$ by 1.5; if it falls below 0.20,
    multiply $\sigma_{\text{prop}}$ by 0.5; otherwise leave it unchanged.
  \item \textbf{Per-coordinate tuning.}  Separate proposal SDs are maintained
    ...
    race dummy).
  \item \textbf{Stopping rule.}  Tuning stops once all parameters fall in the
    target interval, or earlier if at least 90\% of parameter-specific
    acceptance rates are already within range.
\end{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{fig2_trace_plots.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth,height=0.42\textheight,keepaspectratio]{fig_tau_posterior.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.7\textheight,keepaspectratio]{fig4_specialty_random_effects.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.68\textheight,keepaspectratio]{fig5_posterior_predictive.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.55\textheight,keepaspectratio]{fig6_sensitivity_tau.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.62\textheight,keepaspectratio]{fig7_sensitivity_beta.png}
