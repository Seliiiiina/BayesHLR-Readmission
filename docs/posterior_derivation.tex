...
Conditional on $(\alpha,\bbeta,\bu)$, the data $\by$ are independent of $\tau$.
...
all other parameters. This defines a valid Metropolis-within-Gibbs
transition kernel whose stationary distribution is the
...
\begin{enumerate}
  ...
  \[
    \begin{aligned}
      ...
      &\le \underbrace{\int p(\alpha)\,d\alpha}_{=1}\;
...
under this conditional deviance, the likelihood-active nominal parameter
count is 63 ($1 + 43 + 19$). The hyperparameter $\tau$ affects the
posterior through the hierarchical prior but does not enter
\eqref{eq:deviance} directly. Accordingly, we interpret $p_D$ relative to
these 63 likelihood-active parameters; hierarchical shrinkage suggests
$p_D < 63$.
